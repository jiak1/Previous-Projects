...
public class TriangleExplosion : MonoBehaviour
{
    ...
    public void Start()
    {
        StartCoroutine(ExplodeBasicObject(true));
    }

    public IEnumerator ExplodeBasicObject(bool destroy)
    {
    ...
    public IEnumerator ExplodeCharacterObject(bool destroy)
    {
        foreach(SkinnedMeshRenderer mR in gameObject.GetComponentsInChildren<SkinnedMeshRenderer>())
        {
            StartCoroutine(ExplodeCharacterLimb(destroy, mR));
        }
        yield return true;
    }

    public IEnumerator ExplodeCharacterLimb(bool destroy, SkinnedMeshRenderer mr)
    {
